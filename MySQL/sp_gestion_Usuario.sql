...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gestion_Usuario`(IN `opc` VARCHAR(20), IN `p_Id` INT, IN `p_UserName` VARCHAR(30), IN `p_Email` VARCHAR(30), IN `p_Password` VARCHAR(30), IN `p_DateBirth` DATE) NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER BEGIN
